	...
	if (data == dn->dn_phys->dn_blkptr) {
		if (dn->dn_dbuf != NULL)
			...
			rrw_enter(&dmu_objset_ds(dn->dn_objset)->ds_bp_rwlock,
			    RW_READER, FTAG);
	} else if (db != NULL && data == db->db.db_data) {
	...
	if (!dirty &&
		...
		*offset = (*offset & ~(DNODE_BLOCK_SIZE - 1)) +
		...
		for (i = BF64_GET(*offset, 0, epbs);
			...
			if (dirty) {
				if (dnode_block_dirty(dn, lvl - 1, *offset)) {
    ...
    int minlvl, uint64_t blkfill, uint64_t txg)
{
	...
	if (!(flags & DNODE_FIND_DIRTY) && dn->dn_phys->dn_nlevels == 0) {
		error = SET_ERROR(ESRCH);
		goto out;
	}
	...
	maxlvl = (flags & DNODE_FIND_DIRTY) ?
	    dn->dn_nlevels : dn->dn_phys->dn_nlevels;
